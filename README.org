...
| [[https://github.com/company-mode/company-mode][company-mode]]       | 补全框架                             | 用了一段时间 cape.el + corfu，又换回去了                             |
...
- [[https://github.com/minad/cape][cape.el]] + [[https://github.com/minad/corfu][corfu]] 原生 Emacs 实现的补全方案，相比 company 比较轻量化，社区也比较推崇，但是我使用上有两个问题无法避免就回退了：
  1. 会有些莫名其妙的补全，比如数字，在写 SQL 的时候，输入 512，会给一堆的备选项。我给提了 issue：https://github.com/minad/corfu/issues/337 当时有 patch 解决了，
     但是隔了一段时间又复现了。另外在非 lsp 的环境下（React JSX），补全经常卡顿
  2. 配色，备选项选中和没选中区分度很不明显，眼睛看起来很累
