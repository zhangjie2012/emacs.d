...
1. 增加 =read-process-output-max= 和 =gc-cons-threshold= 的值
   #+begin_src elisp
     (setq read-process-output-max (* 1024 1024))  ;; 1mb
     (setq gc-cons-threshold (* 1024 1024 100))  ;; 100mb
   #+end_src
2. 使用 =plists= 反序列化，使用 =lsp-use-plists= 替代默认的 =hash-table=
   1. 添加环境变量 ~export LSP_USE_PLISTS=true~
   2. 删除 =lsp-mode= 相关 package
   3. =lsp-use-plists= 不为 nil
   4. 重启 Emacs，安装 =lsp-mode=
